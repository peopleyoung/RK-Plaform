...
当前仓库离线模板已同步到 `2026.08.26`：平台 API/Web/Media 和训练镜像使用 `2026.08.26`，RK3588 转换/推理镜像及 C++ pipeline 使用 `2026.08.26-business`。升级或生成离线包前，必须重新运行打包脚本、导入对应新镜像，并以新包的 `VERSION`、`manifest.json` 和 `load-images.sh` 校验结果为准。
