...
离线包是独立发布线，当前仓库中的离线默认版本仍为 `2026.08.24`；在线部署使用的 API/Web `2026.08.25` 和 RK3588 `2026.08.25-business` 不能直接填入旧离线包。升级离线包前，必须重新运行打包脚本、导入新镜像，并以新包的 `VERSION`、`manifest.json` 和 `load-images.sh` 校验结果为准。
