...
#include "track/bytetrack/TrackUtil.hpp"

namespace ByteTrack {
/*
- frame_rate 视频流帧率
- track_buffer 跟踪目标丢失后保留的帧数
- track_thresh 区分高置信度和低置信度检测的阈值，高于该阈值的目标添加到 detections 列表中，低于的添加到 detections_low
列表中
- high_thresh  被用于筛选足够高置信度的检测，在未匹配的检测中，置信度高于该阈值的检测才会被激活并加入到
activated_sttracks 列表中
...
void BYTETracker::reset_parameters(int frame_rate, int track_buffer) {
    max_time_lost = int(frame_rate / 30.0 * track_buffer);
}

vector<STrack> BYTETracker::update(const std::vector<Object> &objects) {
    ...
    if (objects.size() > 0) {
        for (int i = 0; i < objects.size(); i++) {
            ...
            auto  frame_target = objects[i].frame_target;

            STrack strack(STrack::tlbr_to_tlwh(tlbr_), score, frame_target);
...
void BYTETracker::linear_assignment(vector<vector<float>> &cost_matrix, int cost_matrix_size, int cost_matrix_size_size,
    ...
    vector<int> rowsol;
    vector<int> colsol;
    lapjv(cost_matrix, rowsol, colsol, true,
          thresh);  // 求解分配任务的算法 reference: https://zhuanlan.zhihu.com/p/571623539
...
vector<vector<float>> BYTETracker::iou_distance(vector<STrack *> &atracks, vector<STrack> &btracks, int &dist_size,
                                                int &dist_size_size) {
...
double BYTETracker::lapjv(const vector<vector<float>> &cost, vector<int> &rowsol, vector<int> &colsol, bool extend_cost,
