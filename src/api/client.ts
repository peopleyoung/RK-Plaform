...
export const api = {
  ...
  inferenceOperatorCatalog: () => request<InferenceOperatorCatalog>('/inference-operator-catalog'),
  validateInferenceGraph: (graph: InferenceGraph, nodeId?: string, taskId?: string, inputUri?: string) => request<InferenceGraphValidation>('/inference-graphs/validate', { method: 'POST', body: JSON.stringify({ graph, nodeId, taskId, inputUri }) }),
  createInferenceTask: (payload: { name: string; nodeId?: string; groupId?: string; inputUri: string; graph: InferenceGraph; layout?: InferenceGraphLayout; npuCoreMask?: InferenceTask['npuCoreMask']; npuCorePolicy?: InferenceTask['npuCorePolicy'] }) => request<InferenceTask>('/inference-tasks', { method: 'POST', body: JSON.stringify(payload) }),
  updateInferenceTask: (id: string, payload: { name: string; nodeId?: string; groupId?: string; inputUri: string; graph: InferenceGraph; layout?: InferenceGraphLayout; baseRevisionId: string; npuCoreMask?: InferenceTask['npuCoreMask']; npuCorePolicy?: InferenceTask['npuCorePolicy'] }) => request<InferenceTask>(`/inference-tasks/${encodeURIComponent(id)}`, { method: 'PUT', body: JSON.stringify(payload) }),
  ...
  createDeployment: (payload: { name: string; taskIds: string[]; strategy: 'canary' | 'rolling' | 'all_at_once'; batchSize?: number }) => request<Deployment>('/deployments', { method: 'POST', body: JSON.stringify(payload) }),
